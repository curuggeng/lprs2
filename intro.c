...
int main(void) {
	...
	while(1){
		...
		case RECT:
			...
			if((gs.rect.x + mov_x*STEP) > (SCREEN_W-RECT_W)) gs.rect.x = SCREEN_W-RECT_W;
			else if((gs.rect.x + mov_x*STEP) < 0) gs.rect.x = 0;
			else gs.rect.x += mov_x*STEP;

			if(((gs.rect.y + mov_y*STEP) > (SCREEN_H-RECT_H))) gs.rect.y = SCREEN_H-RECT_H;
			else if((gs.rect.y + mov_x*STEP) < 0) gs.rect.y = 0;
			else gs.rect.y += mov_y*STEP;
			
			if((gs.rect.y == gs.sq.y) || (gs.rect.y == gs.sq.y)){
				gs.sq.x = SCREEN_W-SQ_A;
				gs.sq.y = SCREEN_H-SQ_A;
			} 
			...
			break;
		case SQ:
			if((gs.sq.x + mov_x*STEP) > (SCREEN_W-SQ_A)) gs.sq.x = SCREEN_W-SQ_A;
			else if((gs.sq.x + mov_x*STEP) < 0) gs.sq.x = 0;
			else gs.sq.x += mov_x*STEP;

			if((gs.sq.y + mov_y*STEP) > (SCREEN_H-SQ_A)) gs.sq.y = SCREEN_H-SQ_A;
			else if((gs.sq.y + mov_x*STEP) < 0) gs.sq.y = 0;
			else gs.sq.y += mov_y*STEP;
