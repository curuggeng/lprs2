...
typedef enum {
	PACMAN_IDLE,
	PACMAN_OPENING_MOUTH_RIGHT,
	PACMAN_WITH_OPEN_MOUTH_RIGHT,
	PACMAN_CLOSING_MOUTH_RIGHT,
	PACMAN_OPENING_MOUTH_LEFT,
	PACMAN_WITH_OPEN_MOUTH_LEFT,
	PACMAN_CLOSING_MOUTH_LEFT,
	...
	while(1){
		...
		case PACMAN_IDLE:
			if(mov_x == 1){
				...
				gs.pacman.anim.state = PACMAN_WITH_OPEN_MOUTH_RIGHT;
			}else if(mov_x == -1){
				gs.pacman.anim.delay_cnt = PACMAN_ANIM_DELAY;
				gs.pacman.anim.state = PACMAN_WITH_OPEN_MOUTH_LEFT;
			}
			if(mov_y == 1){
				gs.pacman.anim.delay_cnt = PACMAN_ANIM_DELAY;
				gs.pacman.anim.state = PACMAN_WITH_OPEN_MOUTH_RIGHT;
			...
			break;
		case PACMAN_OPENING_MOUTH_RIGHT:
			...
			break;
		case PACMAN_WITH_OPEN_MOUTH_RIGHT:
			if(gs.pacman.anim.delay_cnt != 0){
					gs.pacman.anim.delay_cnt--;
			}else{
				if(mov_x != 0 || mov_y != 0){
					gs.pacman.anim.delay_cnt = PACMAN_ANIM_DELAY;
					gs.pacman.anim.state = PACMAN_CLOSING_MOUTH_RIGHT;
				}else{
					gs.pacman.anim.state = PACMAN_IDLE;
				}
			}
			break;
		case PACMAN_WITH_OPEN_MOUTH_LEFT:
			if(gs.pacman.anim.delay_cnt != 0){
					gs.pacman.anim.delay_cnt--;
			}else{
				if(mov_x != 0 || mov_y != 0){
					gs.pacman.anim.delay_cnt = PACMAN_ANIM_DELAY;
					gs.pacman.anim.state = PACMAN_CLOSING_MOUTH_LEFT;
				}else{
					gs.pacman.anim.state = PACMAN_IDLE;
				}
			}
			break;
		case PACMAN_CLOSING_MOUTH_RIGHT:
			if(gs.pacman.anim.delay_cnt != 0){
					gs.pacman.anim.delay_cnt--;
			}else{
				gs.pacman.anim.delay_cnt = PACMAN_ANIM_DELAY;
			...
			break;
		case PACMAN_CLOSING_MOUTH_LEFT:
			if(gs.pacman.anim.delay_cnt != 0){
					gs.pacman.anim.delay_cnt--;
			}else{
				gs.pacman.anim.delay_cnt = PACMAN_ANIM_DELAY;
		...
		case PACMAN_WITH_CLOSED_MOUTH:
			if(gs.pacman.anim.delay_cnt != 0){
				...
				if(mov_x != 0 || mov_y != 0){
					gs.pacman.anim.delay_cnt = PACMAN_ANIM_DELAY;
					gs.pacman.anim.state = PACMAN_OPENING_MOUTH_RIGHT;
				}else{
					gs.pacman.anim.state = PACMAN_IDLE;
				}
			}
		...
		case PACMAN_WITH_OPEN_MOUTH_RIGHT:
			...
			break;
		case PACMAN_OPENING_MOUTH_LEFT:
		case PACMAN_CLOSING_MOUTH_LEFT:
			// Half open mouth.
			draw_sprite_from_atlas(
